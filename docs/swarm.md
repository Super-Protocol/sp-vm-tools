...
There are two main paths to run a Swarm cluster, depending on your goals:
...
| **Full deployment** | `scripts/start_super_protocol.sh` | Production / multi-host setup. |

> Additional scenarios — local image builds and GCP deployment — are covered in [Advanced](#advanced).
...
This guide walks through the full Swarm VM lifecycle on a confidential host: preparing the environment, populating the provider configuration, starting and verifying the VM, and stopping it. It is written around the `gp-ws-01` build server but works on any host that satisfies the prerequisites.
...
**Get the launch tooling.** For a normal launch (downloading the released image) you only need `sp-vm-tools` — it contains `start_super_protocol.sh`. The `sp-vm` repository is the image source and is only needed if you build the image yourself (see [Building the VM image locally](#building-the-vm-image-locally)).
...
The provider configuration is a directory you create anywhere on the host (this guide uses `~/swarm/provider-configs/`). You pass its path to the launch script via `--provider_config`. It contains:
...
- `authorized_keys` — SSH public keys to authorize for the VM (debug mode only; see [Debugging](#debugging)).

#### Bootstrap vs joining nodes

> **Cluster size.** A single bootstrap node will start and is fine for a quick local test, but a real cluster needs a quorum for the consensus layer (Kubernetes/etcd control plane, Swarm DB gossip). Run **at least 3 nodes** (an odd number — 3, 5, …) so the cluster can tolerate a node failure and still reach quorum. Start the bootstrap node first, then join the rest. The Terraform example below reflects this with `node_count = 3`.

| Field | Bootstrap | Joining |
|---|---|---|
| `swarm_db.advertise_addr` | set to this node's public IP | set to this node's public IP; must match `--ip_address` |
...
The bootstrap node starts first. After it is up, note its external IP and the gossip port (default `7946`). Joining nodes use that address in `join_addresses` and must be started with the same `networkID`.

**Fetching `caBundle` from the bootstrap node.** The bootstrap node exposes its PKI authority on port `9443`. Pull the CA bundle and paste it under `pki_authority.caBundle` on every joining node:

```bash
curl -k https://<bootstrap-ip>:9443/api/v1/pki/certs/ca
```

> To forward port `9443` from the host into the VM, start the bootstrap node with `--pki_port 9443` (see the flags table in [Start the Swarm VM](#start-the-swarm-vm)). Without it the PKI port is not forwarded.
...
swarm_db:
  ...
  # bootstrap vs joining: see "Bootstrap vs joining nodes" table above
  join_addresses: []

pki_authority:
  networkID: "aaa-bbb-ccc"          # bootstrap vs joining: see "Bootstrap vs joining nodes" table above
  # bootstrap vs joining: see "Bootstrap vs joining nodes" table above
  caBundle: |
    -----BEGIN CERTIFICATE-----
    .....
...
> Replace `<your-domain>` with a domain you own, `<your-subdomain>` with the Swarm-specific label, and `<your-powerdns-api-host>` / `<your-powerdns-api-key>` with credentials for **your** DNS API.
...
<details>
<summary>Need a GitHub PAT? Expand</summary>
...
#### Copying the configuration to the host
...
By default the script downloads the latest already-built VM image (release) from the Super Protocol release storage — you do **not** need to build anything. The first launch on a fresh host therefore downloads ~11 GB into the cache directory before the VM starts.

> If you need a locally built image instead of the released one, see [Building the VM image locally](#building-the-vm-image-locally) and pass `--build_dir ~/projects/sp-vm/out`.
...
**Start a persistent terminal session.** Run the launch inside a `tmux` (or `screen`) session. The image download and the VM itself are long-running; if your SSH connection drops you'll lose the VM (and any in-progress download) together with it.
...
> By default the latest release is fetched. Pin a specific build with `--release <name>`. Use a locally built image with `--build_dir ~/projects/sp-vm/out` (see [Building the VM image locally](#building-the-vm-image-locally)).

> Set `--ip_address` to the host's public IPv4 (the one other Swarm nodes will reach you on). The default `0.0.0.0` binds every interface, which is fine on a single-homed host but unsafe on multi-NIC machines. Whichever IP you pick must match `swarm_db.advertise_addr` in `config.yaml` (see [Bootstrap vs joining nodes](#bootstrap-vs-joining-nodes) table) and be the address the other nodes' `join_addresses` point to.

> The script auto-detects the confidential-computing mode from the host CPU (`tdx`, `sev-snp`, or `untrusted`). Override it with `--mode <tdx|sev-snp|untrusted>` only if auto-detection picks the wrong one.
...
| `--build_dir` | `./out` | _(none — downloads release)_ | Use a locally built VM image instead of downloading a release (see [Building the VM image locally](#building-the-vm-image-locally)). |
...
| `--swarm-init` | `true` | `false` | Bootstrap a new Swarm cluster. See [Bootstrap vs joining nodes](#bootstrap-vs-joining-nodes) table. |
| `--allow-untrusted` | `true` | `false` | Allow untrusted mode. Can only be combined with `--swarm-init true`. |

### Verify the VM is up

In production mode the VM logs to the serial console attached to your terminal (the script runs QEMU with `-serial stdio`). Watch for these signals that the node is up:

- **Serial console** — services starting up.
- **Swarm Cloud UI and API endpoints** — become reachable once the cluster is running.
...
### Building the VM image locally

By default the launch script downloads the latest released VM image, so most users never need this section. Build locally only when you need a custom or unreleased image; then pass `--build_dir ./out` to the launch command.

**Clone `sp-vm`.** The image is built from the `sp-vm` repository. It depends on `swarm-cloud` via git submodules, so a recursive clone (or `git submodule update --init --recursive`) is mandatory.
...
**Create a buildx builder with `security.insecure`.** The build needs privileged operations inside Docker, so a custom builder is required.
...
**Use the built image.** Pass the build directory to the launch command (it lives at `~/projects/sp-vm/out`):
...
> **Prerequisite:** This path requires a **locally built** raw image. First follow [Building the VM image locally](#building-the-vm-image-locally), then return here. A downloaded release is the packaged QEMU image, not a raw disk, so it cannot be used.

As an alternative to Terraform, `sp-vm-tools/scripts/run_custom_conf_vm.sh` packs a raw VM disk into a GCE image, creates a confidential VM from it, and (optionally) uploads the provider config to a GCS bucket that the VM mounts over s3fs.
...
- The provider config directory from [Provider configuration](#provider-configuration), passed to `--provider-config`.
...
> `PROJECT_ID` defaults to `supa-swarm`; override it with the `PROJECT_ID` environment variable (`PROJECT_ID=my-proj ./run_custom_conf_vm.sh ...`) or by editing the script. The image tarball goes to the bucket from `--bucket` (default `gs://supa-swarm-bucket-conf-vms`).
...
**After launch.** The script enables the serial port and prints connection commands:
...
When a provider config is uploaded, the VM receives S3-style HMAC credentials via instance metadata and mounts the bucket at `/sp/` using s3fs. The exact metadata keys and the s3fs mount command are printed at the end of the run.

> **Tip:** start with `--dry-run` to review every `gcloud`/`gsutil` call (and confirm the resolved project, bucket, and image names) before creating anything in GCP.

---

## Debugging

Debug mode is **not** needed for a normal production launch. Enable it only when you need an interactive shell inside the VM or verbose boot logging — for example while developing or troubleshooting.

Debug mode changes behaviour in a few ways:

- **SSH into the VM** is only available in debug mode. The `authorized_keys` / `ssh_public_keys` fields are honored only here; in a non-debug (production) image the SSH server is disabled and the keys are ignored.
- `--log_file` is **required** when `--debug true` is set, and is **only** allowed in debug mode (passing it without `--debug true` is an error).
...
Same as the [production launch](#start-the-swarm-vm), plus the following flags:
...
Full debug launch command for copy-paste convenience (only the three flags above differ from production):
...
When you have a shell inside the VM (debug mode), the provider configuration is mounted under `/sp/`:
